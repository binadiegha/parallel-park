...
if __name__ == "__main__":
    ...
    model = DQN(
        ...
        verbose=1,
        tensorboard_log="parallel_parking_mlp/",
    )
    model.learn(total_timesteps=int(1e5),    progress_bar=True,)
    model.save("parallel_parking_mlp/model")

    # Record video
    model = DQN.load("parallel_parking_mlp/model")

    env = DummyVecEnv([test_env])
    video_length = 2 * env.envs[0].config["duration"]
    env = VecVideoRecorder(
        env,
        "parallel_parking_mlp/videos/",
