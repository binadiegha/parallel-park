...
class ParallelParkingEnv(AbstractEnv, GoalEnv):
    ...
    def compute_reward(
        ...
        # Add speed penalty - penalize high speeds when close to the goal
        speed_penalty = 0.0
        for vehicle in self.controlled_vehicles:
            current_speed = self._get_vehicle_speed(vehicle)
            
            # Calculate distance to goal for this vehicle
            ...
            if goal_distance < 5.0:
                # Penalty increases as we get closer to goal and speed is higher
                proximity_factor = max(0, (5.0 - goal_distance) / 5.0)  # 0 to 1
                speed_factor = max(0, current_speed - self.config["max_stop_speed"])
                speed_penalty += proximity_factor * speed_factor * self.config["speed_penalty_weight"]
        
        return base_reward - speed_penalty
    ...
    def _is_success(self, achieved_goal: np.ndarray, desired_goal: np.ndarray) -> bool:
        ...
        Success requires both position/orientation accuracy AND complete stop.
        """
        # Check position/orientation accuracy
        position_success = (
        ...
        return position_success and all_stopped
